...
def BC_value(tau, zratio1_1):
    ...
    tau = tau * r_switch.bound_switch(self = r_switch, comb = "10", zratio1 = zratio1, zratio2 = zratio2)
    output = r_sol.batch(self = r_sol, K = tau, comb = "10", zratio1 = zratio1, zratio2 = zratio2, tol = 1e-8)
    return output

tau_grid = numpy.array([-1, *stats.norm.cdf(numpy.linspace(-1.2, 1.2, 41), scale = .5) * 2 - 1, 1], dtype = numpy.double)
zratio1_1_grid = numpy.array([0, *stats.norm.cdf(numpy.linspace(-1.2, 1.2, 41), scale = .5), 1], dtype = numpy.double)
...
def BC_par(i):
    ...
    return out
"""Defining core numbers"""
value_BC = Parallel(n_jobs=4)(delayed(BC_par)(i) for i in range(len(points_BC_tau_grid)))
value_BC = numpy.array(value_BC, dtype=numpy.double).reshape(points_BC_meshgrid[0].shape)
print(value_BC)
