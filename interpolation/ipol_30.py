...
def NC_value(tau, zratio1_1, zratio1_2):
    ...
    tau = tau * r_switch.bound_switch(self = r_switch, comb = "30", zratio1 = zratio1, zratio2 = zratio2)
    output = r_sol.batch(self = r_sol, K = tau, comb = "30", zratio1 = zratio1, zratio2 = zratio2, tol = 1e-8)
    return output

tau_grid = numpy.array([-1, *stats.norm.cdf(numpy.linspace(-1.2, 1.2, 41), scale = .5) * 2 - 1, 1], dtype = numpy.double)
zratio1_1_grid = zratio1_2_grid = numpy.array([0, *stats.norm.cdf(numpy.linspace(-2.1, 2.1, 41)), 1], dtype = numpy.double)
...
def NC_par(i):
    ...
    return out
value_NC = Parallel(n_jobs=4)(delayed(NC_par)(i) for i in range(len(points_NC_tau_grid)))
value_NC = numpy.array(value_NC, dtype=numpy.double).reshape(points_NC_meshgrid[0].shape)
print(value_NC)
