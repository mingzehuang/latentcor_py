...
def NB_value(tau, zratio1_1, zratio1_2, zratio2_1):
    ...
    tau = tau * r_switch.bound_switch(self = r_switch, comb = "31", zratio1 = zratio1, zratio2 = zratio2)
    output = r_sol.batch(self = r_sol, K = tau, comb = "31", zratio1 = zratio1, zratio2 = zratio2, tol = 1e-8)
    return output

tau_grid = numpy.array([-1, *stats.norm.cdf(numpy.linspace(-1.2, 1.2, 25), scale = .5) * 2 - 1, 1], dtype = numpy.double)
zratio1_1_grid = zratio1_2_grid = zratio2_1_grid = numpy.array([0, *stats.norm.cdf(numpy.linspace(-1.8, 1.8, 25), scale = .8), 1], dtype = numpy.double)
