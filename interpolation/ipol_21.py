...
def TB_value(tau, zratio1_1, zratio2_1):
    ...
    tau = tau * r_switch.bound_switch(self = r_switch, comb = "21", zratio1 = zratio1, zratio2 = zratio2)
    output = r_sol.batch(self = r_sol, K = tau, comb = "21", zratio1 = zratio1, zratio2 = zratio2, tol = 1e-8)
    return output
...
def TB_par(i):
    ...
    return out
value_TB = Parallel(n_jobs=4)(delayed(TB_par)(i) for i in range(len(points_TB_tau_grid)))
value_TB = numpy.array(value_TB, dtype=numpy.double).reshape(points_TB_meshgrid[0].shape)
print(value_TB)
